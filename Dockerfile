...
RUN cd /opt/ && \
	...
	cd nrpe-2.15.tar.gz
RUN ./configure --enable-command-args --with-nagios-user=${NAGIOS_USER} --with-nagios-group=${NAGIOS_GROUP} --with-ssl=/usr/bin/openssl --with-ssl-lib=/usr/lib/x86_64-linux-gnu && \
	make all && \
	make install && \
	make install-xinetd && \
